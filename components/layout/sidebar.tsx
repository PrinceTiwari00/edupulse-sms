...
export default function Sidebar({ role }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full w-80 bg-[#0f172a] text-white border-r border-white/5 no-print shrink-0 overflow-hidden shadow-2xl">
      <div className="p-8">
        <Link href="/" className="text-3xl font-black flex items-center gap-4 text-white hover:opacity-90 transition-all">
          <div className="bg-indigo-500 p-2.5 rounded-2xl shadow-xl shadow-indigo-500/20 border border-indigo-400/30">
            <School className="w-8 h-8 text-white" />
          </div>
          <span className="tracking-tighter uppercase">EduPulse</span>
        </Link>
        <div className="mt-8 px-2">
            <p className="text-[11px] text-indigo-300/50 uppercase tracking-[0.3em] font-black leading-none">Management Core</p>
            <div className="h-[2px] w-8 bg-indigo-500 mt-2 rounded-full"></div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-6 py-4 space-y-2 mt-4 overflow-y-auto custom-scrollbar">
        {links.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-6 h-6 shrink-0 transition-colors ${isActive ? 'text-white' : 'text-slate-500 group-hover:text-indigo-400'}`} />
      ...
      <div className="p-8 border-t border-white/5 space-y-3 bg-[#0a0f1d]/50 backdrop-blur-xl">
        <Link
          href="/settings"
          className="flex items-center gap-4 px-5 py-4 rounded-2xl text-slate-400 hover:bg-white/5 hover:text-white transition-all font-black text-sm uppercase tracking-wider group"
        >
          <Settings className="w-6 h-6 text-slate-600 group-hover:text-indigo-400 transition-colors" />
          <span>Settings</span>
        </Link>
        <button
          className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-red-400 hover:bg-red-500/10 transition-all font-black text-sm uppercase tracking-wider group"
