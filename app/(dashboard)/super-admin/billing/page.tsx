...
export default function SaaSInstitutionalBilling() {
  ...
  return (
    <div className="space-y-8 lg:space-y-12 pb-20 px-0 md:px-4">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-2">
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none">Billing Terminal</h1>
          <p className="text-slate-700 font-bold text-sm md:text-lg max-w-xl leading-relaxed">Manage institutional revenue, tiered plans, and subscription payment lifecycles.</p>
        </div>
        <div className="flex flex-wrap gap-4 w-full lg:w-auto no-print">
          <button 
            onClick={() => setShowPlanEditor(true)} 
            className="flex-1 lg:flex-none flex items-center justify-center p-5 bg-white border-2 border-slate-200 rounded-[24px] hover:bg-slate-50 shadow-sm transition-all"
          >
            <Settings className="w-6 h-6 text-slate-700" />
          </button>
          <button 
            onClick={() => setShowInvoicer(true)}
            className="flex-[3] lg:flex-none flex items-center justify-center gap-3 bg-indigo-600 text-white px-10 py-5 rounded-[24px] font-black text-xs md:text-sm hover:bg-indigo-700 shadow-2xl shadow-indigo-100 transition-all uppercase tracking-widest active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
         <div className="bg-white p-8 md:p-10 rounded-[48px] border-2 border-indigo-600 shadow-2xl shadow-indigo-100 relative overflow-hidden group hover:shadow-indigo-200 transition-all">
            <div className="relative z-10">
               <p className="text-xs font-black text-indigo-600 uppercase tracking-[0.3em] mb-6">Total Realized Revenue</p>
               <h2 className="text-4xl md:text-5xl font-black tracking-tighter leading-none text-slate-900">रू {financials.totalCollected.toLocaleString()}</h2>
               <div className="flex items-center gap-2 mt-8 text-indigo-700 font-black text-[10px] uppercase tracking-widest bg-indigo-50 w-fit px-4 py-2 rounded-xl border border-indigo-100">
                  <TrendingUp className="w-4 h-4" /> +12% Net Growth
               </div>
            </div>
            <div className="absolute -top-12 -right-12 w-64 h-64 bg-indigo-50 rounded-full blur-[80px] group-hover:bg-indigo-100 transition-all duration-700"></div>
         </div>
         
         <div className="bg-white p-8 md:p-10 rounded-[48px] border border-slate-200 shadow-sm flex items-center gap-8 group hover:shadow-xl transition-all">
            <div className="p-5 bg-amber-50 rounded-[32px] group-hover:scale-110 transition-transform shadow-sm border border-amber-100">
               <History className="w-8 h-8 text-amber-600" />
            </div>
            <div>
               <p className="text-sm font-black text-slate-500 uppercase tracking-[0.2em] leading-none mb-3">Accounts Receivable</p>
               <p className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter leading-none">रू {financials.totalPending.toLocaleString()}</p>
            </div>
         </div>

         <div className="bg-white p-8 md:p-10 rounded-[48px] border border-slate-200 shadow-sm flex items-center gap-8 cursor-pointer hover:bg-slate-50 transition-all group" onClick={() => setShowPromoSetup(true)}>
            <div className="p-5 bg-indigo-50 rounded-[32px] group-hover:scale-110 transition-transform border border-indigo-100 shadow-sm">
               <Tag className="w-8 h-8 text-indigo-600" />
            </div>
            <div>
               <p className="text-sm font-black text-slate-500 uppercase tracking-[0.2em] leading-none mb-3">Promo Center</p>
               <p className="text-xs font-black text-indigo-700 mt-2 uppercase tracking-widest bg-indigo-50/50 px-5 py-2.5 rounded-xl border border-indigo-200">Issue Discount Code</p>
            </div>
         </div>
      </div>
         </div>

         <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm flex items-center gap-10 cursor-pointer hover:bg-slate-50 transition-all group" onClick={() => setShowPromoSetup(true)}>
            <div className="p-6 bg-indigo-50 rounded-[32px] group-hover:scale-110 transition-transform border border-indigo-100 shadow-sm">
            ...
            <div>
               ...
               <p className="text-sm font-black text-indigo-600 mt-2 uppercase tracking-widest bg-indigo-50/50 px-4 py-2 rounded-xl border border-indigo-100">Issue Discount Code</p>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         <div className="lg:col-span-2 space-y-10">
            <div className="bg-white rounded-[40px] md:rounded-[56px] border border-slate-200 shadow-xl overflow-hidden">
               <div className="p-8 md:p-12 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between bg-slate-50/20 gap-8">
                  <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight uppercase">SaaS Ledger</h3>
                  <div className="relative w-full md:w-96 no-print">
                    ...
                    <input 
                      ...
                      className="w-full pl-14 pr-6 py-5 bg-white border-2 border-slate-100 rounded-[28px] text-base font-bold focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 shadow-sm transition-all"
                    />
                  </div>
               </div>
                  ...
                  <table className="w-full text-left border-collapse min-w-[900px]">
                     <thead>
                        <tr className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] border-b border-slate-100">
                           ...
                           <th className="px-10 py-8">Institution</th>
                           <th className="px-10 py-8 text-right">Net Payable</th>
                     ...
                     <tbody className="divide-y divide-slate-50">
                        {filteredInvoices.length === 0 ? (
                           <tr><td colSpan={6} className="px-10 py-32 text-center text-slate-400 font-black uppercase tracking-widest text-lg italic">No active billing records found.</td></tr>
                           ...
                           return (
                             <tr key={inv.id} className="group hover:bg-slate-50/50 transition-all duration-300">
                                <td className="px-10 py-10 font-mono font-black text-xs text-indigo-700 uppercase tracking-tighter">{inv.invoiceNumber}</td>
                                <td className="px-10 py-10">
                                   <p className="font-black text-slate-900 text-xl tracking-tight leading-none mb-2">{inv.school.name}</p>
                                   {Number(inv.school.walletBalance) > 0 && (
                                      <span className="text-[10px] text-green-700 font-black uppercase bg-green-50 px-3 py-1 rounded-lg border border-green-200">
                                         Wallet: रू {Number(inv.school.walletBalance).toLocaleString()}
                                      </span>
                                   )}
                                </td>
                                <td className="px-10 py-10 font-black text-slate-900 text-xl text-right tracking-tighter leading-none">रू {Number(inv.totalAmount).toLocaleString()}</td>
                                <td className={`px-10 py-10 font-black text-xl text-right tracking-tighter leading-none ${dues > 0 ? 'text-red-600' : 'text-slate-300'}`}>
                                   रू {dues.toLocaleString()}
                                </td>
                                <td className="px-10 py-10 text-center">
                                   <span className={`px-6 py-2.5 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-md border-2 ${
                                   ...
                                   <div className="flex justify-end gap-4">
                                     {inv.status !== 'PAID' && (
                                       <button 
                                          ...
                                          className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-indigo-600 transition-all shadow-xl active:scale-95"
                                       >
                                          Collect
                                       </button>
                                     )}
                                     <button 
                                        ...
                                        className="p-4 bg-white text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 border-2 border-slate-100 rounded-2xl transition-all shadow-sm"
                                        title="Payment History"
                                     >
                                        <History className="w-6 h-6" />
                                     </button>
                                     <button onClick={() => handleDeleteInvoice(inv.id)} className="p-4 bg-white text-slate-200 hover:text-red-500 border-2 border-slate-100 rounded-2xl transition-all shadow-sm">
                                        <Trash2 className="w-6 h-6" />
                                     </button>
                                     <Link href={`/super-admin/billing/invoice/${inv.id}`} className="p-4 bg-white hover:bg-slate-100 rounded-2xl transition-all inline-block border-2 border-slate-100 shadow-sm">
            ...
            <div className="bg-white rounded-[40px] md:rounded-[56px] border border-slate-200 shadow-xl overflow-hidden">
               <div className="p-6 md:p-10 border-b border-slate-100 bg-slate-50/20 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight uppercase">Recent Receipts</h3>
                  <button className="text-[10px] md:text-xs font-black text-indigo-600 uppercase tracking-[0.2em] bg-indigo-50 px-6 py-3 rounded-xl hover:bg-indigo-100 transition-all">Download Audit Ledger</button>
               </div>
               <div className="overflow-x-auto overflow-y-hidden">
         ...
         <div className="space-y-10">
            <div className="bg-white rounded-[48px] border border-slate-200 shadow-xl overflow-hidden">
               ...
               <div className="divide-y divide-slate-100">
                  {promos.length === 0 ? (
                    <div className="p-10 text-center text-xs font-black text-slate-300 uppercase tracking-widest italic leading-none">No discount codes configured.</div>
                  ) : promos.map(promo => (
                    <div key={promo.id} className="p-8 flex items-center justify-between hover:bg-slate-50/50 transition-all">
                       <div>
                          <p className="font-black text-indigo-700 text-xl uppercase tracking-widest leading-none mb-2">{promo.code}</p>
                          <p className="text-xs font-black text-slate-500 uppercase tracking-widest leading-none">Benefit: {promo.discount}{promo.isPercent ? '%' : ' NPR'}</p>
                       </div>
                       <button onClick={() => handleDeletePromo(promo.id)} className="p-5 bg-white text-slate-400 hover:text-red-500 border-2 border-slate-100 rounded-2xl transition-all shadow-sm">
                          <Trash2 className="w-6 h-6" />
                       </button>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-[48px] border-2 border-indigo-600 p-10 shadow-2xl relative overflow-hidden group">
               <div className="flex justify-between items-center mb-12 relative z-10">
                    <h4 className="text-2xl font-black tracking-tighter uppercase leading-none text-slate-900 text-left">Subscription <br/>Tiers</h4>
                    <div className="p-5 bg-indigo-50 rounded-[28px] border border-indigo-100 shadow-sm">
                        <Zap className="w-8 h-8 text-indigo-600" />
                    </div>
               </div>
               <div className="space-y-8 relative z-10">
                  {plans.filter(p => p.name !== 'FREE').map((plan) => (
                     <div key={plan.id} className="p-8 bg-slate-50 rounded-[32px] border-2 border-slate-100 flex justify-between items-center group/item hover:border-indigo-200 transition-all cursor-default shadow-sm">
                        <div>
                           <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mb-2 leading-none">{plan.name}</p>
                           <p className="text-3xl font-black text-slate-900 tracking-tighter leading-none">रू {Number(plan.price).toLocaleString()}</p>
                        </div>
                        <CheckCircle2 className="w-10 h-10 text-indigo-500 group-hover/item:scale-110 transition-transform opacity-40 group-hover/item:opacity-100" />
                     </div>
                  ))}
               </div>
            </div>
