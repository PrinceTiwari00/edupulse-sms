...
export default function SuperAdminSchoolsPage() {
  ...
  return (
    <div className="space-y-12 pb-20">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-xs font-black text-slate-600 uppercase tracking-[0.2em] border-b border-slate-100">
                ...
                <th className="px-4 py-8">Institution</th>
                <th className="px-4 py-8">Subdomain</th>
                <th className="px-4 py-8 text-center">Resources</th>
                <th className="px-4 py-8 text-center">Plan</th>
                ...
                <tr key={school.id} className={`group hover:bg-slate-50 transition-all duration-300 ${selectedIds.includes(school.id) ? 'bg-indigo-50/30' : ''}`}>
                  ...
                  <td className="px-4 py-8">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center font-black text-2xl text-indigo-600 shadow-xl border border-indigo-50 group-hover:scale-105 transition-transform shrink-0">
                        {school.name[0]}
                      </div>
                      <div className="min-w-0">
                        <p className="font-black text-slate-900 text-lg tracking-tight leading-none mb-2 truncate">{school.name}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest truncate">{school.email || 'NO_RECORD'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-8">
                    <div className="flex items-center gap-2 text-[10px] font-mono font-black text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-xl w-fit border border-indigo-100 shadow-sm uppercase tracking-tighter shrink-0">
                    ...
                    <div className="flex flex-col items-center gap-2 shrink-0">
                       <div className="flex items-center gap-3">
                          <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-600 uppercase">
                             <Users className="w-4 h-4 text-indigo-500" /> {school._count.students}
                          </div>
                          <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-600 uppercase">
                             <ShieldCheck className="w-4 h-4 text-green-500" /> {school._count.staff}
                          </div>
                       </div>
                       <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-200">
                          <div className="bg-indigo-600 h-full w-[40%] rounded-full"></div>
                       </div>
                    </div>
                  </td>
                  <td className="px-4 py-8 text-center">
                    <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg border-2 shrink-0 ${
                      school.plan === 'ENTERPRISE' ? 'bg-slate-950 text-white border-slate-800' :
                      school.plan === 'PRO' ? 'bg-indigo-600 text-white border-indigo-500' :
                      school.plan === 'BASIC' ? 'bg-blue-50 text-blue-900 border-blue-200' : 'bg-slate-50 text-slate-400 border-slate-200'
                    }`}>
                      {school.plan || 'NONE'}
                    </span>
                  </td>
                  <td className="px-4 py-8 text-center">
                    <button 
                      ...
                      className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md border-2 shrink-0 ${
                        school.isActive ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'
                      }`}
                    >
                      {school.isActive ? (
                        <>
                  ...
                  <td className="px-6 py-8 text-right relative">
                    <button 
                      ...
                      className="p-3 hover:bg-slate-200 rounded-2xl transition-all inline-flex items-center justify-center border-2 border-transparent hover:border-slate-100"
                    >
      ...
      {manageUserSchool && (
         <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-xl">
            <div className="bg-white rounded-[56px] w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 max-h-[90vh] flex flex-col">
               <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/40">
                  <div>
                    <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter leading-none">Manage Institutional Users</h2>
                    <p className="text-[11px] font-black text-indigo-600 uppercase tracking-[0.3em] mt-2">{manageUserSchool.name} Core Access</p>
                  </div>
                  <button onClick={() => { setManageUserSchool(null); setEditingUser(null); }} className="p-4 hover:bg-slate-200 rounded-3xl transition-all border border-slate-100 shadow-sm">
                    <X className="w-6 h-6 text-slate-400" />
                  </button>
               </div>
               ...
               <div className="flex-1 overflow-y-auto p-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
                  {/* User Form */}
                  <div className="space-y-8 bg-slate-50/50 p-8 rounded-[40px] border border-slate-100 h-fit sticky top-0">
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{editingUser ? 'Update User Identity' : 'Provision New Identity'}</h3>
                    <form onSubmit={handleUserSubmit} className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">First Name</label>
                                <input name="firstName" type="text" defaultValue={editingUser?.firstName} required className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Last Name</label>
                                <input name="lastName" type="text" defaultValue={editingUser?.lastName} required className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Unique Username</label>
                            <input name="username" type="text" defaultValue={editingUser?.username} required className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Communication Email</label>
                            <input name="email" type="email" defaultValue={editingUser?.email} required className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Secret Password {editingUser && '(Leave blank to keep)'}</label>
                            <input name="password" type="password" required={!editingUser} className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Access Role</label>
                            <select name="role" defaultValue={editingUser?.role || 'SCHOOL_ADMIN'} className="w-full px-6 py-4 border-2 border-white rounded-[24px] bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm cursor-pointer">
                                <option value="SCHOOL_ADMIN">INSTITUTION ADMIN</option>
                                <option value="ACCOUNTANT">INSTITUTION ACCOUNTANT</option>
                                <option value="TEACHER">STAFF TEACHER</option>
                            </select>
                        </div>
                        <div className="flex gap-3">
                            <button type="submit" disabled={isUpdating} className="flex-1 bg-slate-900 text-white py-5 rounded-[24px] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-2">
                                {isUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                                {editingUser ? 'Sync Identity' : 'Provision User'}
                            </button>
                            {editingUser && (
                                <button type="button" onClick={() => setEditingUser(null)} className="px-6 py-5 bg-slate-200 text-slate-600 rounded-[24px] font-black text-xs uppercase">Cancel</button>
                            )}
                        </div>
                    </form>
                  </div>
                  ...
                  <div className="space-y-6">
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Active Identity Buffer</h3>
                    <div className="space-y-4">
                        {loadingUsers ? (
                            <div className="py-20 text-center text-slate-300 font-black uppercase tracking-widest animate-pulse">Syncing user database...</div>
                        ) : schoolUsers.length === 0 ? (
                            <div className="py-20 text-center text-slate-300 font-black uppercase tracking-widest italic">No users found for this institution.</div>
                        ) : schoolUsers.map((user: any) => (
                            <div key={user.id} className="p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        <p className="font-black text-slate-900 text-base leading-none mb-1">{user.firstName} {user.lastName}</p>
                                        <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest leading-none mb-1">{user.role.replace('_', ' ')}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase">{user.username}</p>
                                    </div>
                                </div>
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => setEditingUser(user)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-indigo-600 rounded-xl transition-all border border-slate-100"><Edit className="w-4 h-4" /></button>
                                    <button onClick={() => handleDeleteUser(user.id)} className="p-2.5 bg-red-50 text-red-400 hover:text-red-600 rounded-xl transition-all border border-red-100"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            </div>
                        ))}
                    </div>
               ...
               <div className="p-10 bg-slate-950 text-white flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <p className="text-sm font-black uppercase tracking-widest">{schoolUsers.length} Authorized Identities Traceable</p>
                  </div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">Governance Protocol V2.1</p>
               </div>
            </div>
         </div>
      )}
